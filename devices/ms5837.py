...
# adapted to circuitpython for the bar30 depth/pressure sensor referencing the following:
...
class MS5837(Wing):
    ...
    def __init__(self, model=MODEL_30BA):
        self.device = I2CDevice(Wing.i2c, self._MS5837_ADDR)
        ...
        self._D1 = 0
        self._D2 = 0
        self._C = [0 for i in range(8)]
    
    def i2c_write(self, *msg, stop=False):
        with self.device:
            self.device.write(bytes(msg), stop=stop)

    def i2c_read(self, len):
        with self.device:
    ...
    def init(self):
        self.i2c_write(self._MS5837_RESET)
    ...
    def read(self, oversampling=OSR_8192):
        ...
        self.i2c_write(self._MS5837_CONVERT_D1_256 + 2*oversampling)
        ...
        self.i2c_write(self._MS5837_ADC_READ)
        d = self.i2c_read(3)
        self._D1 = d[0] << 16 | d[1] << 8 | d[2]
        
        # Request D2 conversion (pressure)
        self.i2c_write(self._MS5837_CONVERT_D2_256 + 2*oversampling)
